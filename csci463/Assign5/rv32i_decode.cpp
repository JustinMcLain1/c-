...
{
    uint32_t rd_field = get_rd(insn);
    int32_t csr = (insn & 0xfff00000) >> (25-5);
    uint32_t rsOne = get_rs1(insn);

    std::ostringstream os;
    os << render_mnemonic(mnemonic) << render_reg(rd_field) << ",0x" << hex::to_hex0x12(csr |= 0x00000000) << "," << render_reg(rsOne);
...
std::string rv32i_decode::render_csrrxi(uint32_t insn, const char *mnemonic)
{
    uint32_t rd_field = get_rd(insn);
    int32_t rsOne = get_rs1(insn);
    int32_t csr2 = (insn & 0xfff00000) >> (25-5);
    
    
    std::ostringstream os;
    os << render_mnemonic(mnemonic) << render_reg(rd_field) << "," <<hex::to_hex0x12(csr2 |= 0x00000000) << "," << rsOne;
    return os.str();
